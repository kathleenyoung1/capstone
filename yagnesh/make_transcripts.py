    ...
    for docket in saved_dockets:
        ...
        if len(transcript) == len(speakers) == len(speaker_roles) == len(times_new):
            with open(f"{os.getcwd()}/{docket}.txt", "w") as outfile:
                ...
                case_info['start'] = case_info[0].apply(lambda x: x[0])
                case_info['end'] = case_info[0].apply(lambda x: x[-1])
                case_info['speaker'] = case_info[1]
                case_info['speaker_role'] = case_info[2]
                case_info['text'] = case_info[3].apply(lambda x: " ".join(x))

                case_info = case_info[['start','end','speaker','speaker_role','text']]
                case_info.to_json(f"{os.getcwd()}/{docket}.json")
